...
__restore_config() {
  if has_backup_file "$retroarch_config_path"; then
    if [ -f "$retroarch_config_path" ]; then
      # Keep track of the Player 1 non-gun inputs since they're generated by the autoconfig
      grep -E '^input_player1.+' "$retroarch_config_path" | grep -Ev 'gun|mbtn' > "$tmp_ephemeral_dir/retroarch-inputs.cfg"
