...
__configure_emulator_configs() {
  while read -r emulator_filename; do
    local source_file="{system_config_dir}/autoport/emulators/$emulator_filename"
    local target_file="$retropie_system_config_dir/autoport/emulators/$emulator_filename"

    if any_path_exists "$source_file"; then
      ini_merge "$source_file" "$target_file" backup=false overwrite=true
    else
      rm -fv "$target_file"
    fi
