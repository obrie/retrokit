...
__configure_core_options() {
  ...
  while read core_name; do
    ...
    grep -E "^$core_name[\-_]" "$global_core_options_path" > "$tmp_core_options_path" || true
    ...
    each_path '{config_dir}/retroarch/retroarch-core-options.cfg' cat '{}' | grep -E "^$core_name[\-_]" > "$tmp_core_options_path" || true
    if [ -s "$tmp_core_options_path" ]; then
      ini_merge "$tmp_core_options_path" "$core_options_path" backup=false >/dev/null
    fi
