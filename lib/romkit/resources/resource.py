...
class Resource:
    ...
    def install(self, source: Optional[Resource] = None, force: bool = False, **kwargs) -> None:
        ...
        if not self.exists() or (self.is_locally_sourced and self.target_path.path != self.source_url_path and self.source_url_path.exists() and self.install_action.overwrite_target) or (self.download_path.exists() and self.install_action.overwrite_target) or force:
            source.download()
